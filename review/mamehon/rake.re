...
\end{array} 
//}
@<strong>{数乙幾何撃的表現}解析學デハ便宜上自由ニ幾何學ノ術語ヲ流用スル．例ヘバ座標法
...
//texequation{
\sqrt {\left( x@<b>{{1\}-x}{1}'\right) ^{2}+\left( x@<b>{{2\}-x}{2}'\right) ^{2}+\ldots \left( x@<b>{{n\}-x}{n}'\right)^{2}}
//}
ナル数ヲP, P'ノ距離ト略稱シテ，ソレヲPP'ト書ク．然ラバ「三角關係」PP'+P'P"とPP"
ガ成立ツ． 若シモPヲ固定スレバ
//texequation{
PP'^{2}=\left( x@<b>{{1\}-x}{1}'\right) ^{2}+\left( x@<b>{{2\}-x}{2}'\right) ^{2}+\ldots \left( x@<b>{{n\}-x}{n}'\right)^{2} < \delta ^{2}
//}
...
//texequation{
\left| x@<b>{{1\}-x}{1}'\right|  < \delta ,\left| x@<b>{{2\}-x}{2}'\right|  < \delta ,\ldots,\left| x@<b>{{n\}-x}{n}'\right|  < \delta ,
//}
言ヒ換ヘレバ
//texequation{
Max(\left| x@<b>{{1\}-x}{1}'\right|,\left| x@<b>{{n\}-x}{n}'\right|) < \delta ,
//}
...
メルコトガ出来タトスルトキ，コノヤウナ組分ケ（A, B）ヲでできんどノ@<strong>{切断}トイヒ，Aヲ
下組，Bヲ上組トイフ．
...
@<strong>{定理1．}實数ノ切断ハ，下組ト上組トノ境界トシテ，ーツノ数ヲ確定スル．（でできん
どノ定理）
...
〔或ハ下方〕ニ@<strong>{有界}デアルトイヒ，Mヲソノーツノ@<strong>{上界}〔或ハ@<strong>{下界}〕トイフ．上方ニモ下方
ニモ有界ナラバ，單ニ有界トイフ．
...
(1°) Sニ屬スル@<b>{凡テノ}数xニ關シテx≦a.



(2°) a'<aトスレバ，a'<xナル@<b>{或ル}数xガSニ属スル．
...
@<strong>{下限}ニ關シテハ不等記號ノ向キヲ反對ニスレバヨイ．
...
以上，上限下限ノ意味ヲ述ベタガ，次＝ソノ存在ノ證明ヲスル．
 ...
 @<strong>{定理2.} 数ノ集合Sガ上方[又ハ下方]ニ有界ナラバSノ上限[又ハ下限]ガ存在ス
ル．（わいやすとらすノ定理）
...
//footnote[29-1][コノヤウナ記法ハ標語的ニノミ使用スル． 即チ「極限値ガアル」トイフトキ，ソノ極限値ハ＋∞ 又ハ -∞デハナイトスル．ソレラヲモ入レテ言フトキニハ，特別ニ断ワル．]
    @<strong>{定理5.} @<m>{\{a_{n\}}},@<m>{\{b_{n\}}}ガ収斂スルトキ，
